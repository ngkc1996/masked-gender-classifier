...
SAVE_FOLDER_NAME = "masked-images"
MASKED_IMAGE_SUFFIX = '-with-mask'
...
PATTERN_IMAGE_PATH = os.path.join(IMAGE_DIR, 'pattern-mask.png')

random_mask_pool = [BLACK_IMAGE_PATH, BLUE_IMAGE_PATH, PATTERN_IMAGE_PATH]
...
def cli():
    ...
    parser.add_argument('--show', action='store_true',
                        help='Whether show picture with mask or not.')
...
def create_mask(image_path, save_path=None, random_masks=False):
    mask_path = BLUE_IMAGE_PATH if random_masks is False else random.choice(
        random_mask_pool)
    show = False
    model = "hog"
    return FaceMasker(image_path, mask_path, show, model, save_path).mask()


class FaceMasker:
    KEY_FACIAL_FEATURES = ('nose_bridge', 'chin')

    def __init__(self, face_path, mask_path, show=False, model='hog', save_path=None):
    ...
    def _save(self):
        folder_splits = os.path.split(self.face_path)
        path_splits = os.path.splitext(folder_splits[1])

        if self.save_path is None:
            new_face_path = os.path.join(
                folder_splits[0], SAVE_FOLDER_NAME, path_splits[0] + MASKED_IMAGE_SUFFIX + path_splits[1])
        else:
            new_face_path = os.path.join(
                self.save_path, path_splits[0] + MASKED_IMAGE_SUFFIX + path_splits[1])
